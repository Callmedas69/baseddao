...
export default function Hero() {
  ...
  return (
    <div className="bg-transparent max-w-[374px] md:max-w-[500px] lg:max-w-6xl lg:w-[1100px] flex flex-col justify-center items-center lg:flex-row lg:justify-start lg:items-start py-[48px] md:py-[64px] gap-8 md:gap-16 px-4 md:px-10">
      {/* DAO NFT IMAGE */}
      <div className="h-[342px] w-[342px] md:w-[420px] md:h-[420px] relative shrink-0 rounded-[48px] md:rounded-[64px] border-[3px] border-[#e5e3d9] overflow-hidden  flex justify-center items-center">
        {tokenInfo && (
          <Image
            src={tokenInfo?.image}
      ...
      {/* BUTTON ~ PREV BID INFO ~ CURRENT BID INFO ~ WINNING BID INFO */}
      <div className="flex flex-col gap-6 max-w-full overflow-hidden">
        ...
        {/* DAO NFT NUMBER */}
        <h1>Based Dao #{parseInt(tokenId, 16)}</h1>
    ...
    <div className="flex flex-col items-start">
      ...
      >
        ...
        <div className="flex flex-col gap-2 shrink-0 min-w-[165px] md:pr-6">
          ...
          <h3>
            {auctionData
              ? `Ξ ${formatNumber(utils.formatEther(auctionData.amount || "0"), 5)}`
              : "n/a"}
          </h3>
        </div>
    ...
    >
      <div className="flex flex-row flex-wrap md:justify-start w-full gap-4 ">
        <div className="flex flex-col gap-2 md:pr-6 shrink-0 min-w-[165px]">
          ...
          <h3>
            Ξ{" "}
            {formatNumber(utils.formatEther(auctionInfo?.highestBid || "0"), 5)}
          </h3>
        </div>

        <div className="flex flex-col gap-2">
          {/* WINNER ADDRESS AND COUNTDOWN */}
          <div className="font-light">
